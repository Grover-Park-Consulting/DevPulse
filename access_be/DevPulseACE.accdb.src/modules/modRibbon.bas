...
Public gblnMainFormOpen As Boolean
...
110       Select Case CLng(intNodeIndex)
              Case 1:  DoCmd.OpenForm "frmMain", acNormal, , , , acWindowNormal
...
200           Case 21: Call BackupDevPulse
              Case 22:
                  If Not CurrentProject.AllForms("frmMain").IsLoaded Then
                      MsgBox "Return to the DevPulse main screen first.", vbInformation, "DevPulse"
                  Else
                      DoCmd.OpenForm "frmClearExampleData", acNormal, , , , acWindowNormal
                  End If
210           Case 99:
...
270   Resume
End Function
Public Sub Ribbon_GetEnabled(ByVal control As Object, ByRef Enabled As Variant)
    Enabled = gblnMainFormOpen
End Sub
